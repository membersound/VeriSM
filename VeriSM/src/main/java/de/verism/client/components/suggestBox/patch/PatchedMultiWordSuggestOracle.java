...
/** !! IMPORTANT !!
 *  This is the original class taken from GWT 2.5.0.
 *  As of issue #8066 there is a serious bug when displaying only one item:
 *  if the input string goes beyond a multi word string, then the suggestion popup
 *  does still show if only the first words match, and the whole string would not match.
 *  
 *  Root cause is {@link #createCandidatesFromSearch} which breaks for "candidateSet.size() < 2".
 *  The only change here is that "break" has been commented out.
 *  
 *  As this class heavily depends on the package protected {@link PrefixTree}, this class has
 *  also been patched, see  {@link PatchedPrefixTree}.
 */
public class PatchedMultiWordSuggestOracle extends SuggestOracle {

  /**
   * Suggestion class for {@link PatchedMultiWordSuggestOracle}.
   */
   ...
   */
  private final PatchedPrefixTree tree = new PatchedPrefixTree();
   ...
   */
  public PatchedMultiWordSuggestOracle() {
    this(" ");
  }
   ...
   */
  public PatchedMultiWordSuggestOracle(String whitespaceChars) {
